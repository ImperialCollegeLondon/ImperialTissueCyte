...
def preprocess(normalise):
    ...
    if len(training_idx) > 0:
        training_data_all = np.delete(training_data_all, np.array(training_idx), axis=0)
        training_data_all_label = np.delete(training_data_all_label, np.array(training_idx), axis=0)

    if len(test_idx) > 0:
        test_data_all = np.delete(test_data_all, np.array(test_idx), axis=0)
        test_data_all_label = np.delete(test_data_all_label, np.array(test_idx), axis=0)
