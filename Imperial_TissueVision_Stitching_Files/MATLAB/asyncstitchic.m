...
for section = (str2double(startsec):1:str2double(endsec))
    ...
    for layer = (0:1:str2double(zlayers)-1)
        ...
        lasttile = lasttile + str2double(xtiles)*str2double(ytiles) - 1;
        while completelayer == false
            %files = dir(strcat(tcpath,'/',folder,'/*-',num2str(lasttile),'_0',channel,'.tif'));
            if isempty(dir(strcat(tcpath,'/',folder,'/*-',num2str(lasttile),'_0',channel,'.tif'))) == 0
                completelayer = true;
            else
                fprintf('All tiles per layer not complete. Waiting...\n');
                pause(3);
                completelayer = false;
            end
        end
        ...
        for tile = (0:1:str2double(xtiles)*str2double(ytiles)-1)
            ...
            tileimage2 = rot90(imcrop(tileimage, [crop crop croplength croplength]));
        ...
        for tile = (0:1:str2double(xtiles)*str2double(ytiles)-1)
            ...
            imwrite(tileimage2, strcat(tcpath,'/',id,'-Mosaic/Ch',channel,'_Tile_Sections/Tile_Z',ztoken,'_Y',ytoken,'_X',xtoken,'.tif'))
            ...
            if mod(tile+1,str2double(xtiles)*str2double(ytiles)) == 0
                ...
                tilepath = strcat(tcpath,'/',id,'-Mosaic/Ch',channel,'_Tile_Sections');
