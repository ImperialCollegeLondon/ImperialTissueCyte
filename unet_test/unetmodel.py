...
def get_unet(lossfn):
    ...
    model.compile(optimizer = AdaBoundOptimizer(learning_rate=1e-3, final_lr=0.1), loss = [lossfn], metrics = [losses.dice_loss, lossfn])
    # model.compile(optimizer = SGD(lr=1e-4), loss = [lossfn], metrics = [losses.dice_loss, lossfn]) # slow af
